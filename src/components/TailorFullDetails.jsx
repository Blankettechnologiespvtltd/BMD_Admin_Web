...
//     role:     raw.role     || "",
//     status:   raw.active   ? "Active" : (raw.active === false ? "Inactive" : ""),
//     verify:   raw.verify   ? "Verified" : (raw.verify === false ? "Pending" : ""),
//     aadhar:   raw.aadhar   || "",
//     pan:      raw.pan      || "",
//     other:    raw.other    || "",
//   });
...
//   const [kyc,     setKyc]     = useState({ aadhar: null, pan: null, other: null });
//   const [viewing, setViewing] = useState(null);
//   const [saveMsg, setSaveMsg] = useState(null); // null | "success" | "error"
//   const [showDeleteConfirm, setShowDeleteConfirm] = useState(false);
//   const kycCount = Object.values(kyc).filter(Boolean).length;
//   const tailorId = raw.id || "T-0001";
...
//   const validateAll = () => {
//     const e = {}, t = {};
//     ["full_name", "email", "mobile", "address"].forEach(k => {
...
//       await axios.patch(
//         `http://192.168.1.29:8000/api/v1/admin/tailor-applications/${raw.id}/approve`,
//         {},
//         { headers: { Authorization: `Bearer ${token}` } }
//       );
//       alert("Tailor Approved Successfully");
//       navigate("/tailor-applications");
//     } catch (error) {
//       console.error(error);
...
//       const token = localStorage.getItem("token");
//       await axios.patch(
//         `http://192.168.1.29:8000/api/v1/admin/tailor-applications/${raw.id}/reject`,
//         {},
//         { headers: { Authorization: `Bearer ${token}` } }
//       );
...
//               This action cannot be undone. All data for <span className="font-semibold text-gray-700">{form.full_name}</span> will be permanently removed.
//             </p>
...
//                   <InputField icon={User}   name="full_name"  label="Full Name"     placeholder="Enter full name"       value={form.full_name}  onChange={onChange} onBlur={onBlur} error={errors.name}  touched={touched.name}  disabled={!editMode} />
...
//                   <InputField icon={Scissors} name="specialization" label="Specialization" placeholder="e.g. Bridal, Suits" 
//                     value={form.specialization ?? ""} onChange={onChange} onBlur={onBlur} disabled={!editMode} />
//                 </div>
...
//               {/* ── Footer Admin Workflow Actions ── */}
//               <div className="pt-4 border-t border-gray-200 flex flex-wrap items-center justify-between gap-4">
//                 <div className="flex items-center gap-3">
//                   <button
//                     onClick={handleApprove}
//                     disabled={loading}
//                     className="flex items-center gap-2 h-11 px-6 bg-emerald-600 hover:bg-emerald-700 disabled:bg-gray-300 text-white rounded-xl text-sm font-bold transition-all shadow-md shadow-emerald-100"
//                   >
//                     <UserPlus size={16} /> Approve Application
//                   </button>
//                   <button
//                     onClick={handleReject}
//                     disabled={loading}
//                     className="flex items-center gap-2 h-11 px-6 bg-red-600 hover:bg-red-700 disabled:bg-gray-300 text-white rounded-xl text-sm font-bold transition-all shadow-md shadow-red-100"
//                   >
...
//                   <button
//                     onClick={reset}
//                     className="flex items-center gap-2 h-11 px-4 border-2 border-gray-200 hover:bg-gray-50 text-gray-600 rounded-xl text-sm font-semibold transition-all"
//                   >
